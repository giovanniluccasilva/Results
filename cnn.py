...
from keras.layers.core import Dense, Dropout, Activation, Flatten, Merge, Siamese
...
def cnn(list):
    ...
    Y_train_external = np_utils.to_categorical(y_train_external, nb_classes)
    Y_test_external = np_utils.to_categorical(y_test_external, nb_classes)
    ...
    Y_train_internal = np_utils.to_categorical(y_train_internal, nb_classes)
    Y_test_internal = np_utils.to_categorical(y_test_internal, nb_classes)
    ##############################
    ...
    train = mlp.fit([X_train_original, X_train_external, X_train_internal], Y_train_original, batch_size=batch_size,
                    nb_epoch=nb_epoch, show_accuracy=True, verbose=1,  validation_split=0.1, callbacks=[early_stop])
    ...
    mlp.save_weights('Models/weights_' + str(nb_filters_1) + '_' + str(nb_filters_2) + '_' + str(nb_hidden_neurons) +
                     '_' + str(cont) + '.h5')
